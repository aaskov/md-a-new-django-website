...
## Write a minimal form in polls/tempaltes/polls/detail.html
```html
...
### Include this line in polls/urls.py
`path("<int:question_id>/vote/", views.vote, name="vote"),`

### Crate a real polls/views.py 
```python
...
### Make sure votes can be handled
```python
from django.shortcuts import get_object_or_404, render
...
### And create a results template in polls/templates/polls/resutls.html
```html
<h1>{{ question.question_text }}</h1>
...
### Amend views in the polss/views.py
```python
...
### Create a test to expose the time bug in polls/tests.py
```python
import datetime
...
### Fixing the bug in polls/models.py
```python
def was_published_recently(self):
    now = timezone.now()
...
### More comprehensive tests in polls/tests.py
```python
...
### Now fix that polls that aren't published isn't showed in polls/views.py
```python 
class IndexView(generic.ListView):
    template_name = "polls/index.html"
    context_object_name = "latest_question_list"
...
### Amend the get_queryset  method in polls/views.py
```python
from django.utils import timezone
...
def get_queryset(self):
...
### Continue create tests in polls/test.py
```python
from django.urls import reverse
...
